...
description: "Plane Finderは2009年にシンプルな「地図上の飛行機」アプリとして始まり、Appleのエコシステムに長期的に賭けてネイティブ開発とファーストパーティツールを活用しながら、わずか8人のチームで世界中の受信機ネットワークを構築..."
date: 2026-06-15T18:05:08+09:00
categories: ["テクノロジー"]
...
## AIによる要約
Plane Finderは2009年にシンプルな「地図上の飛行機」アプリとして始まり、Appleのエコシステムに長期的に賭けてネイティブ開発とファーストパーティツールを活用しながら、わずか8人のチームで世界中の受信機ネットワークを構築し、フルスタックの飛行追跡ビジネスへと進化した。このアプローチはプラットフォームへの依存と早期技術採用が小規模チームでもグローバル規模でのサービス提供を可能にすることを示し、開発者コミュニティからのフィードバックが技術の正しい適用と製品改善に不可欠であることを裏付けている。したがって、ネイティブプラットフォームに特化し、エコシステムのツールを最大限に活用し、ユーザーとの双方向の関係を築くことが、少人数でも持続可能な成長とイノベーションを実現する鍵である。
...
*Plane Finder*は、小さなチームがプラットフォームとともに成長すると何が起こるかの輝かしい例である。

2009年にリリースされた*Plane Finder*は、人員を増やしたり、ベンダーを追加したり、複雑さを増すことによって年々スケールアップしたわけではない。代わりに、創業者のジョディーとリー・アームストロングは、Appleのエコシステムへの長期的な賭けに出た——ネイティブのままにし、ファーストパーティツールに近づき、プラットフォームのシグナルを早期に読み取ることに注力した。そして時間が経つにつれ、「地図上の飛行機」から始まったアプリは、世界中の物理ハードウェアネットワークを含むフルエンドツーエンドの飛行追跡ビジネスへと進化した——たった8人のチームによって構築・運営されている。

私たちは夫婦である創業者に、初期の日々、新しいデザインとLiquid Glass、そしてグローバルな飛行追跡ネットワークを運営する上での課題について話を聞いた。
...
**2009年に戻って、Plane Finderのアイデアは何から生まれ、当時の日々はどんな感じでしたか？**

**Lee:** 私たちはApp Storeがオープンしてから約1年後に参加しています。まるで一生分のように感じます。しかし本当の火種は、2007年に発表されたiPhoneそのものを見たことでした。実際に発売時にアメリカにいたので、手に入れましたが、それに何をするかはまだはっきりしていませんでした。当時はまだApp Storeがなく、英国では電話として使うことすらできませんでした。ただ持ってスワイプするだけのものでした。しかしその瞬間は私たちの旅の大きな一部となりました。今でもそのiPhoneをディスプレイに飾っています。

**当時、エンドツーエンドの飛行追跡プラットフォームになることを望んでいましたか？**

**Jodie:** 全くそんなことはありませんでした。最初はただのアプリから始めました。今では航空機から直接位置情報を収集し、アプリ内に組み込んで商業的にデータを販売しています。

**8人の小さなチームですが、それはどんな感じですか？**

**Lee:** Appleの技術がなければ、これができなかったと思います。私たちは小さなチームなので、App Store——クレジットカード処理、StoreKit、ローカライズ——がなければ、グローバル規模でマーケティングするためのプラットフォームや方法を持っていなかったでしょう。私たちは本当にApp Storeというプラットフォームを価値あるものと考えています。

![](/rss/downloaded_images/68beb02574f96c0a17cbc64e29432ccb.jpeg)

*Plane Finder*は、ARKit、MapKit、Liquid GlassといったAppleの技術や機能を早期に採用していることで知られています。どのツールが最も大きな違いを生み出しましたか？

**Lee:** すべてはMapKitに遡ります。私たちは冗談交じりにアプリを「地図上の飛行機」と言いますが、MapKitはその核です。また、3D地球ビューのためにMetalを大いに活用しています。StoreKit 2がなければ、サブスクリプションやプロモーションオファーを含む収益化を処理することはできなかったでしょう。サードパーティやクロスプラットフォームフレームワークは一切使っていません。Appleの技術に全力で取り組んでいるのは、それが私たちが必要とするすべてを提供してくれるからです。

**なぜそんなに早期導入に前向きになれたのですか？**

**Jodie:** 私は、昔聞いた言葉「新しい技術が現れたとき、あなたは蒸しローラーになるか、道路になるかのどちらかだ」という考え方で会社を導いています。私たちは常に蒸しローラー側になりたいと思っています。新しい技術を素早く評価し、製品に意味のある形で活用できると判断すれば、すぐに導入します。

**Liquid Glassの導入プロセスについて話せますか？**

**Jodie:** そのコンセプトにはすぐに賛同しました。リーダーシップの観点から、「これは未来だ。私たちのやっていることに意味を持たせなければならない」と言いました。デザインチームとエンジニアリングチームは、最新状態を保ちながら技術に積極的に取り組み、私たちの世界に合うようにするために本当に一生懸命働きました。

**開発者コミュニティはあなたにとって何を意味しますか？**

**Lee:** それは補強の要素です。サイロで作業していると、コミュニティが正しく技術を適用しているという自信を与えてくれます。WWDCのセッションでスライドやサンプルコードを見るのは良いですが、それは非常に具体的です。実際にどのように機能しているかを見ることは貴重です。

**Jodie:** Appleの中で話す人みんなは、私たちのアプリに情熱と意見を持っています。彼らは非常に関与しており、あらゆるフィードバックは貴重です。これまでに「ツールバーでなぜこうするのですか？」といった質問を受けてきましたが、そんな会話すべてが役に立っています。

![A photo of six members of the Plane Finder team, all standing outside in a courtyard next to an office building.](https://devimages-cdn.apple
...
![Two iPhone screenshots of the app Plane Finder. The screenshot on the left shows a map of the San Francisco area and information about a flight from JFK to SFO. The screenshot on the right show a map of all plane traffic activity over Europe.](https://devimages-cdn.apple.com/wwdc-services/articles/images/6D023485-2D1B-4230-BED4-9E926A75CC9E/2048.jpeg)
