...
description: "Appleは2026年2月24日から、ブラジル、オーストラリア、シンガポール、ユタ、ルイジアナでの年齢確認義務に対応するため、Declared Age Range APIのベータ版アップデートを提供し始めた。これにより、開発者はユーザ..."
date: 2026-06-15T18:12:39+09:00
categories: ["テクノロジー"]
tags: ["年齢確認", "アプリストア", "法規制", "ブラジル", "ユタ"]
---

## AIによる要約
Appleは2026年2月24日から、ブラジル、オーストラリア、シンガポール、ユタ、ルイジアナでの年齢確認義務に対応するため、Declared Age Range APIのベータ版アップデートを提供し始めた。これにより、開発者はユーザーの年齢カテゴリを取得し、18歳以上向けアプリのダウンロード制限やロットボックスを含むアプリの年齢レーティング自動更新など、地域ごとの法規制に合わせた対応が可能になる。特にユタとルイジアナでは新規Appleアカウント作成者に対して年齢情報の共有が求められ、重要なアップデート通知を大人向けに表示する機能もベータで利用できる。これらの変更は、開発者に追加の実装負荷をもたらすと同時に、未成年者への不適切コンテンツへのアクセスを減らし、より安全で年齢に適した体験を提供する方向に業界を導くものと考えられる。したがって、開発者は早期に新しいAPIを導入し、ヒューマンインターフェースガイドラインに従ってアップデートの説明を明確にし、地域ごとの法令順守とユーザー保護の両立を図るべきである。
...
## 和訳本文
## ブラジル、オーストラリア、シンガポール、ユタ、ルイジアナで配布されるアプリの年齢要件

2026年2月24日

今日、ブラジル、オーストラリア、シンガポール、ユタ、ルイジアナを含む今後の米国および地域の法律に基づく年齢確認義務を満たすために開発者が利用できるツールのアップデートを提供します。Declared Age Range APIのアップデートは現在ベータ版としてテストが可能です。

**ブラジル**

ブラジルでアプリを配布する開発者は、アップデートされたDeclared Age Range APIを使用してユーザーの年齢カテゴリを取得できます。ブラジルのユーザーの年齢カテゴリは、ユーザー自身または親または保護者（該当する場合）が年齢カテゴリを共有することに同意したときに共有されます。APIはまた、ユーザーのデバイスから年齢確認の方法に関するシグナルも返します。

**オーストラリア、シンガポール、ブラジルでの18+評価アプリ**

2026年2月24日から、Appleはオーストラリア、ブラジル、シンガポールのユーザーが、合理的な方法で成人であることが確認されていない限り、18+と評価されたアプリをダウンロードできなくします。App Storeはこの確認を自動で行います。ただし、開発者は自分のユーザーが成人であることを独立して確認する別の義務がある場合があります。これに対応するため、iOS、iPadOS、macOSで利用可能なDeclared Age Range APIは、開発者にユーザーの年齢に関する役立つシグナルを提供します。

ブラジルでアプリを配布する開発者が、年齢レーティングの質問票を通じて自分のアプリにロットボックスが含まれていることを特定した場合、そのアプリのブラジルストアfrontでの年齢レーティングは18+に更新されます。

**ユタとルイジアナ**

2026年5月6日にユタで新規Appleアカウントを作成したユーザー、および2026年7月1日にルイジアナで新規Appleアカウントを作成したユーザーについては、Declared Age Range APIを通じてリクエストされたときに年齢カテゴリが開発者のアプリに共有されます。以前に発表したツールは、ルイジアナとユタでのコンプライアンス義務を満たすために拡張され、以下を含みます：

[PermissionKitにおけるSignificant Change API](https://developer.apple.com/documentation/PermissionKit/SignificantAppUpdateTopic)

[StoreKitにおける新しい年齢レーティングプロパティタイプ](https://developer.apple.com/documentation/storekit/appstore/ageRatingCode)

[App Store Server Notifications](https://developer.apple.com/documentation/appstoreservernotifications/notificationtype)

Declared Age Range APIを通じて新たに利用可能なシグナルには、年齢関連の規制要件がユーザーに適用されるかどうか、およびユーザーが年齢範囲を共有する必要があるかどうかが含まれます。また、子供に対する重要なアップデートについて親または保護者の許可が必要かどうかもAPIから知ることができます。

開発者は、Declared Age Range APIを使用して、これらの州の大人に対して Significant Update Action（現在ベータ版）を通じて重要なアップデート通知を表示できます。重要なアップデートをリリースする際は、ヒューマンインターフェースガイドラインに従い、ユーザーにアップデートの意味のある説明を提供しなければなりません。

[あなたのアプリとゲームのために安全で年齢に適した体験を設計する](https://developer.apple.com/kids/#age-assurance)
