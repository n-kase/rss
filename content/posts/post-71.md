...
description: "AMDの自動更新ツールであるAMD Auto Updaterに、HTTPSで取得する更新情報とは別に、実際にダウンロードする実行ファイルのURLがHTTPで提供され、さらにダウンロード後のファイルに暗号学的な署名検証が行われないという..."
date: 2026-06-16T08:26:48+09:00
categories: ["テクノロジー"]
tags: ["AMD", "脆弱性", "セキュリティ", "CVE", "Hacker News"]
image: "/rss/downloaded_images/ef2fcb3126da8318cd5e7ccbc1df7b20.jpg"
---

## AIによる要約

AMDの自動更新ツールであるAMD Auto Updaterに、HTTPSで取得する更新情報とは別に、実際にダウンロードする実行ファイルのURLがHTTPで提供され、さらにダウンロード後のファイルに暗号学的な署名検証が行われないという脆弱性が見つかりました。これにより、同じネットワーク上にいる攻撃者や通信経路上でデータを書き換えることができる中間者が、正規の更新ファイルを悪意のある実行ファイルに差し替えることが可能となり、ユーザーが通常の自動更新と見なしている間にスパイウェアやランサムウェアが管理者権限で実行されるリスクが指摘されました。

セキュリティ研究者のMrBruh氏は2026年1月27日にこの問題を発見し、2月6日にAMDへ報告しましたが、AMDのバグ報奨金プログラムを担当するIntigritiは同日、中間者攻撃を前提とするため報奨金対象外とし、報告をクローズしました。その後、MrBruh氏がブログ記事を公開しHacker Newsで話題になると、AMDのPSIRTは改めて調査すると連絡し、内部では脆弱性としての妥当性を確認すると説明、CVEの発行や修正、研究者へのクレジット付与を行う方針を示しました。一方で、AMDはMrBruh氏に対してブログ記事の削除を要求し、正式対応が終わるまで公開を待つよう求めました。MrBruh氏はこれに従ったものの、その後の連絡は積極的ではなく、最終的に初回報告から124日後の2026年6月9日に公開となりました。AMDは最終的にこの問題を「AMD Auto Updater Vulnerability」として公表し、CVE-2026-40677を割り当て、影響製品としてAMD Management Console、AMD Ryzen Master、AMD µProfを挙げ、それぞれのバージョンアップを緩和策として推奨しました。

この一連の流れは、バグ報奨金制度の運用ルールと実際の脆弱性の深刻さの間に生じるズレを浮き彫りにしました。Hacker Newsでは、中間者攻撃を理由に報奨金対象外とする判断への批判が相次ぎ、HTTPで実行ファイルを配布し署名検証がないことはソフトウェア更新機構として重大な欠陥であるという指摘が多数見られました。また、バグ報奨金制度の対象外という判断が必ずしも脆弱性の影響がないことを意味しないという見方や、中間者攻撃は特定の報奨金プログラムの目的から外れているだけで、脅威そのものが無視できるわけではないというコメントもありました。さらに、YouTubeチャンネル・Gamers NexusもAMDの対応を不誠実かつルールの後付け変更のように批判し、研究者への信頼回復の必要性を訴えました。

この事例から、ソフトウェアベンダーは報奨金プログラムの基準だけに頼らず、セキュリティ報告の内容そのものを慎重に評価し、中間者攻撃などの現実的な脅威軽視を避けるべきであるという教訓が得られます。また、研究者側も報奨金制度の判断に不満がある場合でも、適切なチャネルを通じて情報を公開し、コミュニティの検証を得ることが重要であり、透明性と迅速な対応が信頼関係を保つ鍵となります。です。

[🔗 元記事を読む](https://gigazine.net/news/20260615-amd-auto-updater-vulnerability-rce/)
