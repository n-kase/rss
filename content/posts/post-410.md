---
title: "Price updates for apps, In-App Purchases, and subscriptions"
description: "概要と事実 2026年1月29日にAppleは、App Storeにおけるアプリおよびアプリ内課金の価格調整を発表しました。この調整は、各国の税制変更や為替レートの変動に対応するために行われます。具体的には、ブータンで物品サービス税（..."
date: 2026-06-16T10:47:12+09:00
categories: ["テクノロジー"]
tags: ["App Store", "税金", "価格更新", "国際", "開発者"]
image: "/rss/downloaded_images/cbbc31616b431a9282b1de88227a7793.png"
---

## AIによる要約

### 概要と事実
2026年1月29日にAppleは、App Storeにおけるアプリおよびアプリ内課金の価格調整を発表しました。この調整は、各国の税制変更や為替レートの変動に対応するために行われます。具体的には、ブータンで物品サービス税（GST）5％が導入され、フィンランドではニュース・雑誌・書籍・オーディオブックに適用される付加価値税（VAT）が14％から13.5％に引き下げられました。ガーナでは2019年のCOVID-19ヘルスリカバリーレヴィーが撤廃され、カザフスタンではVATが12％から16％に引き上げられました。リトアニアでは同様のニュース関連商品に対するVATが9％から5％に低下し、モーリシャスではVAT15％が新たに導入されました。ロシアではVATが20％から22％に、トルコではデジタル販売税が7.5％から5％に、ジンバブエではVATが15％から15.5％にそれぞれ引き上げられました。これらの税制変更に伴い、AppleはPaid Applications Agreementの展示Bを更新し、ブータンとモーリシャスにおいて税金の徴収・納付を行うことを明示しています。また、2月16日からはモーリシャスをベースストアフロントとして選択していない開発者については、価格が自動的に更新されますが、ベースストアフロントとして選択している場合や、価格を手動で管理している場合は変更されません。自動更新サブスクリプションについては価格変更の対象外です。

### ネットの反応と意見の変遷
この発表を受けて、開発者コミュニティでは税制変更に対応するための作業負担が増えることへの懸念が示されました。特に、複数の国で税率が頻繁に変動する中で、価格の自動調整機能が正しく機能するかどうかを注視する声が上がっています。一方で、消費者側からは価格が変動しても自動で均等化されるため、購入時の混乱が少なくなるという期待も見られました。SNSやフォーラムでは、税金の導入や撤廃がアプリの価格に直接影響する仕組みに対して、透明性が高いと評価する意見と、頻繁な変更が開発者にとって負担になるという批判的な意見が交錯しています。時間の経過とともに、開発者は価格設定のツールを活用して柔軟に対応する方法を共有し始め、徐々に実務的な対応策が浸透してきたと考えられます。

### 背景と結論
Appleはグローバルに展開するApp Storeにおいて、各国の税法や為替レートの変動に応じて価格を均等に保つ仕組みを維持しています。これにより、開発者は税制変更に応じた価格調整を自動的に行うことができ、消費者は国によって不公平な価格差が生じにくくなります。今回の税制変更は、各国が財政政策やデジタル課税を見直す動きの一環であり、今後も同様の調整が続く可能性があります。開発者にとっては、税制の動向を把握し、価格設定の自動化ツールを適切に活用することが重要となります。一方で、消費者にとっては価格の安定性が保たれることで、アプリやサービスへのアクセスが公平であるというメリットがあります。総じて、税制変更への迅速な対応がプラットフォームの信頼性を支える重要な要素であるということが改めて確認できました。

[🔗 元記事を読む](https://developer.apple.com/news/?id=gvnljl3f)

<details>
<summary>AI要約用RAWデータ（抽出された元記事テキスト）</summary>



![Price updates for apps, In-App Purchases, and subscriptions](/rss/downloaded_images/d20649009dd819404ef35d58a59a66a6.png)



## Price updates for apps, In-App Purchases, and subscriptions

January 29, 2026

The App Store is designed to make it easy to sell your digital goods and services globally, with support for 43 currencies across 175 storefronts.

From time to time, we need to adjust prices or your proceeds due to changes in tax regulations or foreign exchange rates. These adjustments are made using publicly available exchange rate information from financial data providers to help make sure prices for apps and In-App Purchases stay consistent across all storefronts.

**As of January 29:**

Your proceeds from the sale of eligible apps and In‑App Purchases have been modified in:

- Bhutan: Goods and Services Tax (GST) introduction of 5%
- Finland: Reduced value‑added tax (VAT) rate decrease from 14% to 13.5% for news, magazines, books, and audiobooks
- Ghana: Removal of 2019 COVID-19 Health Recovery Levy
- Kazakhstan: VAT rate increase from 12% to 16%
- Lithuania: Reduced VAT rate decrease from 9% to 5% for news, magazines, books, and audiobooks
- Mauritius: VAT introduction of 15%
- Russia: VAT rate increase from 20% to 22%
- Türkiye: Digital sales tax (DST) rate decrease from 7.5% to 5%
- Zimbabwe: VAT rate increase from 15% to 15.5%

Exhibit B of the Paid Applications Agreement will be updated to indicate that Apple collects and remits applicable taxes in Bhutan and Mauritius.¹

**Beginning February 16:**

Pricing for apps and In-App Purchases will be updated for Mauritius if you haven’t selected Mauritius as the base storefront for your app or In‑App Purchase.² This update considers the VAT introduction listed in the tax updates section above.

If you’ve selected Mauritius as the base storefront for your app or In-App Purchase, prices won’t change. On other storefronts, prices will be updated to maintain equalization with your chosen base price.

Prices won’t change in any region if your In‑App Purchase is an auto‑renewable subscription. Prices also won’t change on the storefronts where you manually manage prices instead of using the automated equalized prices.

The Pricing and Availability section of Apps has been updated in [App Store Connect](https://appstoreconnect.apple.com/login) to display these upcoming price changes. As always, you can change the prices of your apps, In‑App Purchases, and auto‑renewable subscriptions at any time.

**Learn more about managing your prices**

[View or edit upcoming price changes](https://developer.apple.com/help/app-store-connect/manage-app-pricing/schedule-price-changes#view-or-edit-upcoming-price-changes)

[Edit your app’s base country or region](https://developer.apple.com/help/app-store-connect/manage-app-pricing/set-a-price#edit-your-apps-base-country-or-region)

[Pricing and availability start times by country or region](https://developer.apple.com/help/app-store-connect/reference/app-store-pricing-and-availability-start-times-by-country-or-region/)

[Set a price for an In-App Purchase](https://developer.apple.com/help/app-store-connect/manage-in-app-purchases/set-a-price-for-an-in-app-purchase)

**Learn more about your proceeds**

¹ Translations of the updated agreement will be available on the [Apple Developer website](https://developer.apple.com/support/terms#paid-applications-agreement) within one month.

² Excludes auto-renewable subscriptions.

</details>
