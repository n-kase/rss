...
## AIによる要約
AMDの自動更新ツールが更新情報を取得する際はHTTPSを使っているものの、実際にダウンロードする実行ファイルのURLはHTTPで、さらにダウンロード後のファイルに暗号学的な署名検証が行われない仕組みだったため、同じネットワーク上にいる攻撃者が通信を書き換えて悪意のある実行ファイルに差し替えることが可能となり、管理者権限でスパイウェアやランサムウェアが実行されるリスクが指摘されました。

セキュリティ研究者のMrBruh氏がこの脆弱性をAMDに報告したものの、バグ報奨金プログラムを担当するIntigritiは中間者攻撃を前提とするため報奨金対象外とし、当初は修正も行われませんでした。

その後、研究者がブログで公開しHacker Newsなどで話題になると、AMDは改めて調査し、CVEを発行して修正バージョンをリリースするとともに研究者へのクレジット付与を表明しましたが、ブログ記事の削除を求めるなど対応に不透明さが残り、最終的に初回報告から約4か月後に公開となりました。

この一連の流れは、ソフトウェアの更新メカニズムにおいてHTTPSと適切な署名検証が必須であることを改めて示し、バグ報奨金制度の運用や研究者への対応の透明性が求められるという教訓を与えています。
